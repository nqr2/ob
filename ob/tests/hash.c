...
void known_results() {
  ...
  ASSERT_HASH_EQ(hash_literal(""), FNV_OFFSET);
  ASSERT_HASH_EQ(hash_literal("a"), 0xaf63dc4c8601ec8c);
  ASSERT_HASH_EQ(hash_literal("foobar"), 0x85944171f73967e8);

  // From the author:
  // http://www.isthe.com/chongo/tech/comp/fnv/index.html
  const uint8_t bytes[] = {0xd5, 0x6b, 0xb9, 0x53, 0x42, 0x87, 0x08, 0x36};
  ASSERT_HASH_EQ(hash_start(8, bytes), 0);
}

void hash_continue_works() {
  auto foo = hash_literal("foo");
  auto foobar = hash_continue(foo, 3, "bar");

  ASSERT_HASH_EQ(foobar, hash_literal("foobar"));
}
